    ...
    <div class="p-6">
      ...
      <div class="flex flex-wrap md:flex-nowrap md:justify-between mb-4 items-center gap-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <Link
            :href="route('devices.create', persistQuery())"
      ...
      <table class="min-w-full table-auto border-collapse border border-gray-300">
        ...
        <tbody>
          <tr v-for="device in devices.data" :key="device.id" class="odd:bg-white even:bg-gray-100">
            ...
            <td class="px-3 py-2 text-center flex justify-center space-x-1">
              ...
              <Link :href="route('devices.edit', { device: device.id, ...persistQuery() })" class="text-blue-500 hover:text-blue-700">
                <PencilIcon class="w-4 h-4"/>
              </Link>
              <button @click="deletedevice(device.id)" class="text-red-500 hover:text-red-700">
                <TrashIcon class="w-4 h-4"/>
              </button>
            </td>
          </tr>
      ...
      <!-- ページネーション -->
      <Pagination :paginator="devices" :onPageChange="goPage" :startItem="startItem" :endItem="endItem"/>
    </div>
  </AppLayout>
</template>
...
const selectAll = computed({
  get() {
    return selectedIds.value.length === props.devices.data.length
  }
})
...
const deletedevice = (device_id) => {
  ...
  router.delete(route('devices.destroy', device_id), {
    preserveState: true,
    onSuccess: () => {
      router.get(route('devices.index'), { ...persistQuery(), page: props.devices.current_page }, { preserveState: true })
    }
  })
}
// 複数削除
